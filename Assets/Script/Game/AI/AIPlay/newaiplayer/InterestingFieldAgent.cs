...
public class InterestingFieldAgent : ICloneable
{
	...
	public ArrayList InterestingFields()
	{
		...
		Console.WriteLine("interestingfields.GetLength(0)=> " + interestingfields.GetLength(0)+ ", interestingfields.GetLength(1)=> " + interestingfields.GetLength(1));
	...
	public void UpdateInterestingFieldArray(int[,] board, Coordinate field)
	{
		...
		int opMatdelta = opMat.GetLength(0) / 2;// 2.5
		Console.WriteLine("opMatdelta=> "+ opMatdelta);

		int x = field.X;
		int y = field.Y;
		Console.WriteLine("x=> " + x + ", y=> " + y);
		for (int j = 0; j < opMat.GetLength(0); ++j) //5번
		{
			for (int k = 0; k < opMat.GetLength(1); ++k) //5번
			{
				if (x + (j - opMatdelta) < board.GetLength(0) && 
					x + (j - opMatdelta) >= 0 && 
					y + (k - opMatdelta) < board.GetLength(0) &&
					y + (k - opMatdelta) >= 0) 
				{
