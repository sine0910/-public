...
//			statValues[i] = /*2 * ownposs + */4 * opp_cutoff  + ownrowbonus + win;
	...
	public int smstatVal(int[,] board, Coordinate node, int attacker, int userX, int userY, ArrayList white_illegal_move_list)
	{
		...
		int digit3 = 0;
		int digit4 = 0;
		//처음  node의 값을 받고 그 주변에 있는 돌을 찾는다. 1234*1234
		bool digit1_1 = false;
		bool digit2_1 = false;
		bool digit3_1 = false;
		bool digit4_1 = false;
		...
		for (int j = 0; j < 9; ++j)
		{
			if (j + node.X - 4 >= 0 && j + node.X - 4 < 15)//좌우로 살피기
			{
			...
			//=================
			if (j + node.Y - 4 >= 0 && j + node.Y - 4 < 15)//위 아래로 살피기
			{
	...
	public int statVal(int[,] board, Coordinate node, int attacker, ArrayList white_illegal_move_list)
	{
		...
		*/
		...
		//Console.WriteLine("statVal의 반환 값=> " + (bonus + (tmpval * turn)));
